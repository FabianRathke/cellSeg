...
for runClass in range(1):
    ...
    dataset = loadData.Dataset(train_data, s_trans, t_trans, st_trans, args.dataAugm, True, args.imgWidth, maskConf = [1,1,0])
    dataloader = torch.utils.data.DataLoader(dataset, num_workers = 2, batch_size = 4)

    validset = loadData.Dataset(val_data, s_trans, t_trans, st_trans, args.dataAugm, True, args.imgWidth, maskConf = [1,1,0])
    ...
    model = UNet2(3,2,learn_weights=args.learnWeights, softmax=True) # Kaggle notebook implementation
